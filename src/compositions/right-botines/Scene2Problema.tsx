...
const OldDesign: React.FC = () => (
  ...
  >
    ...
    <div
      style={{
        background: "linear-gradient(135deg, #b8866a, #d4a07a)",
        height: 90,
        borderRadius: 8,
        display: "flex",
        alignItems: "center",
...
const NewDesign: React.FC = () => (
  ...
  >
    ...
    >
      ...
      <div style={{ flex: 1 }} />
      <div style={{ background: C.cyan, width: 28, height: 28, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 14, height: 12, border: `2px solid ${C.bg}`, borderRadius: 2, position: "relative" }}>
          <div style={{ position: "absolute", top: -6, right: -2, width: 8, height: 8, borderRadius: "50%", background: C.orange }} />
        </div>
      </div>
    </div>
    {/* Ticker */}
...
export const Scene2Problema: React.FC = () => {
  ...
  const titleOp = interpolate(frame, [15, 42], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const subtitleOp = interpolate(frame, [35, 65], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });

  const leftSpr = spring({ frame: frame - 18, fps, config: { damping: 16, stiffness: 90 } });
  const rightSpr = spring({ frame: frame - 32, fps, config: { damping: 16, stiffness: 90 } });

  const arrowProg = interpolate(frame, [65, 130], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  const arrowLabelOp = interpolate(frame, [120, 145], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ background: C.bg, opacity: op }}>
      ...
      >
        <div style={{ fontSize: 52, fontWeight: 900, fontFamily: FONT, color: C.white, lineHeight: 1 }}>
          El Problema
        </div>
        <div
          style={{
      ...
      >
        ...
        <div
          style={{
            ...
            flexDirection: "column",
            gap: 14,
            transform: `translateX(${interpolate(leftSpr, [0, 1], [-80, 0])}px)`,
            opacity: leftSpr,
          }}
        >
          <div
            ...
            {arrowProg > 0.85 && (
              <div
                style={{
                  ...
                  borderLeft: `13px solid ${C.orange}`,
                }}
              />
            )}
          </div>
        ...
        <div
          style={{
            ...
            flexDirection: "column",
            gap: 14,
            transform: `translateX(${interpolate(rightSpr, [0, 1], [80, 0])}px)`,
            opacity: rightSpr,
          }}
        >
          <div
